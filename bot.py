...
def create_user(username, password):
    ...
    try:
        if db_handler.use_postgresql:
            cursor = db_handler.execute(db, 'INSERT INTO users (username, password_hash) VALUES (%s, %s) RETURNING id',
                                        (username, hashed_pw))
            user_id = db_handler.get_lastrowid(cursor, db)
        else:
            cursor = db_handler.execute(db, 'INSERT INTO users (username, password_hash) VALUES (%s, %s)',
                                        (username, hashed_pw))
...
def get_stats(user_id, period):
    ...
    period_start_str = period_start.isoformat() if db_handler.use_postgresql else period_start.strftime(
        '%Y-%m-%d %H:%M:%S')
...
async def show_tasks(update: Update, context: ContextTypes.DEFAULT_TYPE, page=0):
    ...
    for task in page_tasks:
        status_emoji = "✅" if task['computed_status'] == 'done' else "⏳" if task[
                                                                                'computed_status'] == 'in_progress' else "📋"
