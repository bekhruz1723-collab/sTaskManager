...
DATABASE_URL = os.environ.get('DATABASE_URL', 'postgresql://localhost/taskmanager')
...
def create_user(username, password):
    ...
    try:
        cursor.execute('INSERT INTO users (username, password_hash) VALUES (%s, %s) RETURNING id', (username, hashed_pw))
...
def toggle_task(task_id, user_id):
    ...
    cursor.execute('SELECT * FROM tasks WHERE parent_id = %s', (task_id,))
    subtasks = cursor.fetchall()
    
    new_status = 'not_started' if task['status'] == 'done' else 'done'
    completed_at = datetime.utcnow() if new_status == 'done' else None
    
    cursor.execute('UPDATE tasks SET status = %s, completed_at = %s WHERE id = %s', 
...
def toggle_subtask(subtask_id, user_id):
    db = get_db()
    cursor = db.cursor()
    cursor.execute('SELECT * FROM tasks WHERE id = %s', (subtask_id,))
    task = cursor.fetchone()
...
def delete_task(task_id, user_id):
    ...
    if task and task['user_id'] == user_id:
        cursor.execute('DELETE FROM tasks WHERE id = %s OR parent_id = %s', (task_id, task_id))
        db.commit()
        cursor.close()
        db.close()
        return True
...
def get_stats(user_id, period):
    ...
    for task in all_tasks:
        cursor.execute('SELECT * FROM tasks WHERE parent_id = %s', (task['id'],))
        subtasks = cursor.fetchall()
        if subtasks:
    ...
    productivity_query = cursor.fetchall()
    ...
    cursor.execute(f'''
        ...
        LIMIT 5
    ''', (user_id,))
    top_periods_query = cursor.fetchall()
...
async def login_password(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user and verify_password(user, password):
        ...
        await update.message.reply_text(t(lang, 'error_login_invalid'))
        return AUTH_CHOICE
...
async def register_password(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id:
        ...
        await update.message.reply_text(t(lang, 'error_user_exists'))
        return REGISTER_USERNAME
...
async def toggle_subtask_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    db = get_db()
    cursor = db.cursor()
    cursor.execute('SELECT parent_id FROM tasks WHERE id = %s', (subtask_id,))
